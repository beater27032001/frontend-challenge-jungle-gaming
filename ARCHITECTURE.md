...
9. **Causa raiz do vazamento no logout não encontrada.** Dois investigadores
   ...
   `queryClient.clear()`. O conserto em vigor — `clear()` +
   `window.location.href = '/'` — **garante** o requisito eliminatório do §11; o
   custo é perder a suavidade de SPA no logout. Vale investigar com calma depois
   da entrega.
