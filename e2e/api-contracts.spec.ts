...
test.describe('Fix Plan (iteration 2): NftSummary derived fields stay coherent', () => {
  ...
  test('seed remains unchanged: fresh reset top-level fields match the known fixture values, SEED_VERSION is 6', async ({
    ...
    // Fase 7: `QuoteItem` — e portanto `Order.items`, o recibo — ganha
    // `imageUrl`, para o snapshot do pedido carregar a arte em vez de a tela
    // derivar a URL do catálogo (fase 7 decisão 46).
    // Fase 8: `username`/`ensName` no usuário e `type`/`referralCode` na
    // carteira (spec 08 §2.4 e §3.5).
    // Integração: as duas fases subiram para 6 em paralelo, cada uma com um
    // conjunto de campos. Um localStorage marcado como 6 pode ter vindo de
    // qualquer uma e estar faltando metade, então a primeira versão que contém
    // as duas mudanças é a 7. Atualizado, não deletado — mesmo precedente das
    // fases 2, 3 e 4.
    expect(dbDump.seedVersion).toBe(7)
